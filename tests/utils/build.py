...
class BuildTestUtils:
    ...
    def get_expected_build_artifacts() -> Dict[str, str]:
        ...
        return {
            ...
            'cpp_tests': str(cmake_dir / 'tests' / 'cpp' / 'tiny_torch_cpp_tests')
        }
    ...
    def validate_build_artifacts() -> Dict[str, bool]:
        ...
        for name, pattern in artifacts.items():
            ...
            # Special handling for Python extension - try to auto-fix if missing
            if name == 'python_extension' and not results[name]:
                print(f"Python extension not found, attempting auto-fix...")
                if BuildTestUtils.auto_copy_python_extension():
                    ...
                    if '*' in pattern:
                        ...
                        if parent_dir.exists():
                            ...
                            results[name] = len(matches) > 0
                            if results[name]:
                                print(f"✓ Python extension auto-fix successful")
                    else:
                        results[name] = Path(pattern).exists()
                        if results[name]:
                            print(f"✓ Python extension auto-fix successful")
    ...
    @staticmethod
    def auto_copy_python_extension() -> bool:
        """Automatically copy Python extension from build directory to package directory."""
        try:
            import glob
            import shutil
            ...
            # Look for built extension in build directory
            build_pattern = str(cmake_dir / "_C*.so")
            build_files = glob.glob(build_pattern)
            
            if build_files:
                target_dir = project_root / "tiny_torch"
                target_dir.mkdir(exist_ok=True)
                
                for build_file in build_files:
                    target_path = target_dir / Path(build_file).name
                    shutil.copy2(build_file, target_path)
                    print(f"Copied {build_file} -> {target_path}")
                
                return True
            else:
                print(f"No extension files found in {cmake_dir}")
                return False
