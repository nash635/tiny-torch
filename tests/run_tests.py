...
from tests.utils import TestEnvironment, CommandRunner
...
class TestRunner:
    ...
    def __init__(self):
        ...
        # Use longer timeout for system and performance tests
        self.runner = CommandRunner(default_timeout=300)  # 5 minutes
    ...
    def run_cpp_tests(self, verbose: bool = False) -> Dict[str, Any]:
        ...
        cpp_test_binary = self.env.get_cmake_dir() / "tests" / "cpp" / "tiny_torch_cpp_tests"
    ...
    def _print_result(self, category: str, result: Dict[str, Any]):
        ...
        if result.get('skipped'):
            print(f"SKIPPED {category.title()}: {result.get('message', '')}")
        elif result.get('success'):
            print(f"PASSED {category.title()}")
        else:
            print(f"FAILED {category.title()}: {result.get('message', '')}")
